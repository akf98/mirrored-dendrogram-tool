...
def setSubWeights(f):
    global subWeightsDef
    global weightsDef

    titles = getLabels1(f)

    weightsDef = [1 / len(titles) for a in range(len(titles))]
    subWeightsDef = [[] for a in range(7 if f.upper() == 'C' else 8)]

    subtitles = getLabels2(f)
    subtitles[0].pop(0)
    for i in range(len(subWeightsDef)):
        leng = len(subtitles[i])
        subWeightsDef[i] = [1 / leng for a in range(leng)]


def getLabels1(t, original=False):
    tree = ET.parse(base_root + 'SmartHealthVisualization\\Data\\{0}1.xml'.format(t.upper()))
    root = tree.getroot()
    labels = []
...
def getLabels2(t, original=False):
    tree = ET.parse(base_root + 'SmartHealthVisualization\\Data\\{0}1.xml'.format(t.upper()))
    root = tree.getroot()
    l = []
    a = []
...
def calc_sim(f, a, b, weights=None, subWeights=None):
    global subWeightsDef
    global weightsDef
    ...
    if subWeights == None:
        subWeights = subWeightsDef

    if weights == None:
        weights = weightsDef

    tree1 = ET.parse(base_root + 'SmartHealthVisualization\\Data\\{0}{1}.xml'.format(f.upper(), a))
    tree2 = ET.parse(base_root + 'SmartHealthVisualization\\Data\\{0}{1}.xml'.format(f.upper(), b))
    tree_max = ET.parse(base_root + 'SmartHealthVisualization\\Data\\{0}{1}.xml'.format(f.upper(), 'Max'))
    ...
    sim = []
    ind = 0
    raw_similarity_values = [[] for a in range(len(getLabels1(f)))]
    ...
    for child1, child2, childMax in zip(root1, root2, root_max):
        ...
        for e1, e2, eMax in zip(child1, child2, childMax):
            ...
            if e1.tag == 'DOB':
                z = similarityDOB(s1, s2, s_max)
                l2sim += z
                l2sim_det = z
            elif s1.replace('.', '', 1).isdigit() and s2.replace('.', '', 1).isdigit():
                x = similarityNum(float(s1), float(s2), float(s_max))
                l2sim += x
                l2sim_det = x

            elif e1.tag == 'Triggers' or e1.tag == 'Type_of_Abortive_Treatment_':
                q = similarityMultiple(s1, s2)
                l2sim += q
                l2sim_det = q

            else:
                y = similarityText(s1, s2)
                l2sim += y
                l2sim_det = y
...
def get_random_patients():
    ...
    for i in range(NB_ELEMENTS):
       filesNum.append(random.randint(1, 114))
    return filesNum
...
def create_patient_distance_matrix(f, weights=None, subweights=None):
    ...
    # filesNum = [1+a for a in range(NB_ELEMENTS)]
    ...
    n = len(filesNum)
    print('filesNum: ', filesNum)
    m = np.zeros([n, n])
    for i in range(n):
        for j in range(0, i):
            m[i, j] = 1 - round(calc_sim(f, filesNum[i], filesNum[j], weights, subweights), 6)
...
# def getThresholdsList(sim_level):
# 	a= []
# 	a.insert(0,0)
# 	a.insert(1,sim_level[int(len(sim_level)*0.75)-1])
# 	a.insert(2,sim_level[int(len(sim_level)*0.5)-1])
# 	a.insert(3,sim_level[int(len(sim_level)*0.25)-1])
#
# 	return a
...
def plot_dends(models, zoom_percentage1=100, zoom_percentage2=100):
    ...
    # print('here')
    plt.subplot(121)
    dendrogram(getLinkageMatrix(models[0]), orientation='left',
    ...
    plt.subplot(122)
    dendrogram(getLinkageMatrix(models[1]), orientation='right',
