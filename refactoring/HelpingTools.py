...
class OptimalVisualization:
    ...
    def calculate_scores(self):
        ...
        # writers.append(pd.ExcelWriter(os.path.join(file_directory,'distance_matrices_of_model_1.xlsx'), engine='xlsxwriter'))
        # writers.append(pd.ExcelWriter(os.path.join(file_directory,'distance_matrices_of_model_2.xlsx'), engine='xlsxwriter'))


        for i in range(2):
            # print('For model: {0} :'.format(i))
            for x in range(1, self.NB_ELEMENTS + 1):
                d = sch.dendrogram(xp.getLinkageMatrix(models[i]), orientation='left', p=x, truncate_mode='lastp',
                                   no_plot=True)
